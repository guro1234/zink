...
const ChatComponents = ({selectedUser , setSelectedUser}) => {
  ...
  return selectedUser? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <img src={assets.help_icon} alt="" className='max-md:hidden max-w5' />
      </div>
      {/* ----------char area--------- */}
      ...
      p-3 pb-6">
        {messagesDummyData.map((messagesDummyData,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !=="680f50e4f3cd28382ecf9" &&'flex-row-reverse'}`}>
            {
              msg.image?(
              <img src={msg.image} className='max-w-[230px] border border-gray-700
              rounded-lg overflow-hidden mb-8'/>  
              ):(
                <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg
                  mb-8 break-all bg-violet-500/30 text-white ${msg.senderId==='680f50e4f3cd28382ecf9' ?'rounded-br-none':'rounded-bl-none'}`}>{msg.text}</p>
              )
            }
